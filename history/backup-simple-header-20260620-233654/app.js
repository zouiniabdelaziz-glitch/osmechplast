...
const CONFIG = {
  defaultLang: 'de',
  siteVersion: '20260620-review-umbau-v1',
};
...
function markActiveNav() {
  ...
  document.querySelectorAll('.nav a, .dropdown a, .oncc-nav a, .oncc-mega a, .oncc-mobile-menu a').forEach(a => {
...
function initHeaderNav() {
  ...
  const megaButton = document.querySelector('[data-mega-toggle]');
  const megaRoot = megaButton?.closest('.oncc-nav__mega');
  if (megaButton && megaRoot) {
    megaButton.addEventListener('click', event => {
      event.preventDefault();
      const isOpen = megaRoot.classList.toggle('is-open');
      megaButton.setAttribute('aria-expanded', String(isOpen));
    });

    document.addEventListener('click', event => {
      if (!megaRoot.contains(event.target)) {
